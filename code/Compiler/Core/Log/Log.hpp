...
public:
	...
	static void log(const std::string_view& tag, bool newLine, const std::string_view& fmt, T&&... args)
	{
		if(newLine)
		{
			writeLine("{} > " + std::string(fmt), std::string(tag), args...);
		}
		else
		{
			append("{} > " + std::string(fmt), std::string(tag), args...);
		}
	}

	static void trace(const std::string_view file, int line, const std::string_view function, const std::string& message)
	{
...
private:
	...
	{
		std::printf(fmt.data(), std::make_format_args(args...));
		std::printf("\n");
	}
